...
function bindEvents() {
  ...
  document.querySelector("#clearProgress").addEventListener("click", () => {
    localStorage.removeItem("ortocast-czquk-stats");
...
function questionTemplate(item, mode, isSentence) {
  ...
  return `
    <article class="question-card" data-mode="${mode}" data-id="${item.id}">
      ...
      <div class="mini-actions" aria-label="Elegir c, z, qu o k">
        ...
        <button class="pick-button pick-qu" type="button" data-pick="qu">QU</button>
        <button class="pick-button pick-k" type="button" data-pick="k">K</button>
      </div>
...
function renderMasked(text, item, isSentence = false) {
  ...
  const aria = `casilla vacía: elige ${displayCase === "upper" ? "C, Z, QU o K mayúscula" : "c, z, qu o k minúscula"}`;
  const blank = `<span class="blank" data-answer="" data-case="${displayCase}" aria-label="${aria}"><span class="blank-placeholder" aria-hidden="true">${displayCase === "upper" ? "QU" : "qu"}</span></span>`;
...
function setAnswer(blank, letter) {
  if (!["c", "z", "qu", "k"].includes(letter)) return;
  ...
  blank.classList.toggle("is-qu", letter === "qu");
  blank.classList.toggle("is-k", letter === "k");
...
function checkSet(mode) {
  ...
  cards.forEach((card) => {
    ...
    if (!selected) {
      feedback.innerHTML = "<strong>Falta responder.</strong> Pulsa C, Z, QU o K antes de corregir esta palabra.";
      return;
    }
    answeredCards += 1;
...
function highlightNorm(text, ruleId, targetWord = "") {
  ...
  const rules = {
    "c-ca-co-cu": /(ca|co|cu)/gi,
    "qu-que-qui": /(que|qui)/gi,
    "c-ce-ci": /(ce|ci)/gi,
    "z-za-zo-zu": /(za|zo|zu)/gi,
    "z-final-ces": /(z|ces)$/gi,
    "z-azo-aza": /(azo|aza)$/i,
    "z-ez-eza": /(ez|eza)$/i,
    "z-c-verbos-zar": /(z|c)/gi,
    "c-cion": /ción$/i,
    "k-prestamos": /k/gi,
    "homophones": /(c|z|qu|k)/gi,
  };

  const pattern = rules[ruleId] || /(c|z|qu|k)/gi;
...
function loadStats() {
  ...
  try {
    return { ...fallback, ...JSON.parse(localStorage.getItem("ortocast-czquk-stats")) };
  } catch {
    return fallback;
  }
}
...
function saveStats() {
  localStorage.setItem("ortocast-czquk-stats", JSON.stringify(state.stats));
}
