...
function bindEvents() {
  ...
  document.querySelector("#clearProgress").addEventListener("click", () => {
    localStorage.removeItem("ortocast-bv-stats");
...
function checkSet(mode) {
  ...
  cards.forEach((card) => {
    ...
    if (!selected) {
      feedback.innerHTML = "<strong>Falta responder.</strong> Pulsa B o V antes de corregir esta palabra.";
      return;
    }
    answeredCards += 1;
...
function highlightNorm(text, ruleId, targetWord = "") {
  ...
  const rules = {
    ...
    "v-ivoro": /(ívoro|ívora|íbora)$/i,
    "v-andar-estar-tener": /v/gi,
    "v-adv": /adv/gi,
    "v-despues-di": /^(div|dib)/i,
    "homophones": /[bBvV]/,
  };
...
function loadStats() {
  ...
  try {
    return { ...fallback, ...JSON.parse(localStorage.getItem("ortocast-bv-stats")) };
  } catch {
    return fallback;
  }
}
...
function saveStats() {
  localStorage.setItem("ortocast-bv-stats", JSON.stringify(state.stats));
}
