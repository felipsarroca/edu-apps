...
function bindEvents() {
  ...
  document.querySelector("#clearProgress").addEventListener("click", () => {
    localStorage.removeItem("ortocast-h-stats");
...
function renderStudy() {
  ...
  const ruleBadge = rule.letter === " " ? "Se escribe sin H" : `Se escribe con ${rule.letter.toUpperCase()}`;
  els.ruleCard.innerHTML = `
    <div class="rule-meta">
      <span class="badge">${state.ruleIndex + 1} / ${state.rules.length}</span>
      <span class="badge">${ruleBadge}</span>
    </div>
...
function questionTemplate(item, mode, isSentence) {
  ...
  return `
    <article class="question-card" data-mode="${mode}" data-id="${item.id}">
      ...
      <div class="mini-actions" aria-label="Elegir h o sin hache">
        <button class="pick-button pick-h" type="button" data-pick="h">H</button>
        <button class="pick-button pick-none" type="button" data-pick=" ">sin H</button>
      </div>
...
function renderMasked(text, item, isSentence = false) {
  ...
  const aria = `casilla vacía: elige ${displayCase === "upper" ? "H o sin hache" : "h o sin hache"}`;
  const blank = `<span class="blank" data-answer="" data-case="${displayCase}" aria-label="${aria}"><span class="blank-placeholder" aria-hidden="true">${displayCase === "upper" ? "H" : "h"}</span></span>`;
...
function setAnswer(blank, letter) {
  if (!["h", " "].includes(letter)) return;
  blank.dataset.answer = letter;
  blank.innerHTML = "";
  blank.textContent = letter === " " ? " " : (blank.dataset.case === "upper" ? letter.toUpperCase() : letter);
  blank.classList.toggle("is-h", letter === "h");
  blank.classList.toggle("is-none", letter === " ");
...
function checkSet(mode) {
  ...
  cards.forEach((card) => {
    ...
    if (!selected) {
      feedback.innerHTML = "<strong>Falta responder.</strong> Pulsa H o sin H antes de corregir esta palabra.";
      return;
    }
    answeredCards += 1;
...
function highlightNorm(text, ruleId, targetWord = "") {
  ...
  if (targetWord) {
    return escaped.replace(
      ...
      (match) => `<span class="highlighted-word"><span class="norm-highlight">${match}</span></span>`
    );
  }
  ...
  const rules = {
    "h-hie-hue": /^h(ie|ue)/i,
    "h-hum": /^hum/i,
    "h-prefijos": /^(hemi|hexa|hepta|hetero|homo|hiper|hipo|hidro)/i,
    "h-verbos": /^h/i,
    "h-interjecciones": /^h/i,
    "h-familias": /^h/i,
    "no-h-a-ha": /^h/i,
    "no-h-e-he": /^h/i,
    "no-h-ay-hay": /^h/i,
  ...
  const pattern = rules[ruleId] || /^h/i;
...
function saveStats() {
  localStorage.setItem("ortocast-h-stats", JSON.stringify(state.stats));
}
