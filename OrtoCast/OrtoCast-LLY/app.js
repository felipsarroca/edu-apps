...
function bindEvents() {
  ...
  document.querySelector("#clearProgress").addEventListener("click", () => {
    localStorage.removeItem("ortocast-lly-stats");
...
function questionTemplate(item, mode, isSentence) {
  ...
  return `
    <article class="question-card" data-mode="${mode}" data-id="${item.id}">
      ...
      <div class="mini-actions" aria-label="Elegir ll o y">
        <button class="pick-button pick-ll" type="button" data-pick="ll">LL</button>
        <button class="pick-button pick-y" type="button" data-pick="y">Y</button>
      </div>
...
function renderMasked(text, item, isSentence = false) {
  ...
  const aria = `casilla vacía: elige ${displayCase === "upper" ? "LL o Y mayúscula" : "ll o y minúscula"}`;
  const blank = `<span class="blank" data-answer="" data-case="${displayCase}" aria-label="${aria}"><span class="blank-placeholder" aria-hidden="true">${displayCase === "upper" ? "LL" : "ll"}</span></span>`;
...
function setAnswer(blank, letter) {
  if (!["ll", "y"].includes(letter)) return;
  ...
  blank.classList.toggle("is-y", letter === "y");
...
function checkSet(mode) {
  ...
  cards.forEach((card) => {
    ...
    if (!selected) {
      feedback.innerHTML = "<strong>Falta responder.</strong> Pulsa LL o Y antes de corregir esta palabra.";
      return;
    }
    answeredCards += 1;
...
function highlightNorm(text, ruleId, targetWord = "") {
  ...
  const rules = {
    "ll-illo-illa": /(illo|illa|illos|illas)$/i,
    "ll-alle-elle-ello-ella": /(alle|elle|ello|ella|alles|elles|ellos|ellas)$/i,
    "ll-verbos-ll": /ll/gi,
    "ll-familias": /ll/gi,
    "ll-inicial": /^ll/i,
    "y-final": /y$/i,
  ...
  const pattern = rules[ruleId] || /(ll|y)/gi;
...
function loadStats() {
  ...
  try {
    return { ...fallback, ...JSON.parse(localStorage.getItem("ortocast-lly-stats")) };
  } catch {
    return fallback;
  }
}
...
function saveStats() {
  localStorage.setItem("ortocast-lly-stats", JSON.stringify(state.stats));
}
