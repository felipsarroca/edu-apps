...
function bindEvents() {
  ...
  document.querySelector("#clearProgress").addEventListener("click", () => {
    localStorage.removeItem("ortocat-gjtg-stats");
...
function questionTemplate(item, mode, isSentence) {
  ...
  return `
    <article class="question-card" data-mode="${mode}" data-id="${item.id}">
      ...
      <div class="mini-actions" aria-label="Tria g, j, tg o tj">
        ...
        <button class="pick-button pick-tg" type="button" data-pick="tg">TG</button>
        <button class="pick-button pick-tj" type="button" data-pick="tj">TJ</button>
      </div>
...
function renderMasked(text, item, isSentence = false) {
  ...
  const aria = `casella buida: tria ${displayCase === "upper" ? "G, J, TG o TJ majúscula" : "g, j, tg o tj minúscula"}`;
  const blank = `<span class="blank" data-answer="" data-case="${displayCase}" aria-label="${aria}"><span class="blank-placeholder" aria-hidden="true">${displayCase === "upper" ? "TG" : "tg"}</span></span>`;
...
function setAnswer(blank, letter) {
  if (!["g", "j", "tg", "tj"].includes(letter)) return;
  ...
  blank.classList.toggle("is-tg", letter === "tg");
  blank.classList.toggle("is-tj", letter === "tj");
...
function checkSet(mode) {
  ...
  cards.forEach((card) => {
    ...
    if (!selected) {
      feedback.innerHTML = "<strong>Falta respondre.</strong> Tria G, J, TG o TJ abans de corregir aquesta paraula.";
      return;
    }
...
function loadStats() {
  ...
  try {
    return migrateStats(JSON.parse(localStorage.getItem("ortocat-gjtg-stats")), fallback);
  } catch {
    return fallback;
  }
}
...
function saveStats() {
  localStorage.setItem("ortocat-gjtg-stats", JSON.stringify(state.stats));
}
