...
function bindEvents() {
  ...
  document.querySelector("#clearProgress").addEventListener("click", () => {
    localStorage.removeItem("ortocat-bv-stats");
...
function renderStudy() {
  ...
  els.ruleCard.innerHTML = `
    ...
    <div>
      <h3>Exemples</h3>
      <ul class="examples">${rule.examples.map((word) => `<li>${highlightStudyExample(word, rule.id)}</li>`).join("")}</ul>
    </div>
...
function renderMasked(text, item, isSentence = false) {
  ...
  const aria = `casella buida: tria ${displayCase === "upper" ? "B o V majúscula" : "b o v minúscula"}`;
  const blank = `<span class="blank" data-answer="" data-case="${displayCase}" aria-label="${aria}"><span class="blank-placeholder" aria-hidden="true">${displayCase === "upper" ? "B" : "b"}</span></span>`;
...
function setAnswer(blank, letter) {
  if (!["b", "v"].includes(letter)) return;
  blank.dataset.answer = letter;
  blank.innerHTML = "";
  blank.textContent = blank.dataset.case === "upper" ? letter.toUpperCase() : letter;
  blank.classList.toggle("is-b", letter === "b");
  blank.classList.toggle("is-v", letter === "v");
...
function checkSet(mode) {
  ...
  cards.forEach((card) => {
    ...
    if (!selected) {
      feedback.innerHTML = "<strong>Falta respondre.</strong> Tria B o V abans de corregir aquesta paraula.";
      return;
    }
...
function highlightRuleText(text, ruleId, letter = "") {
  ...
  if (ruleId === "b-radical-ava") {
    html = html
      ...
      .replace(
        ...
        () => `la <strong class="explicit-rule-v">v</strong> de l'imperfet`
      )
  ...
  const replacements = {
    ...
    "b-bio-biblio-bi": ["bio-", "biblio-", "bi-"],
    "b-frequents": ["baca", "bell", "beure", "botar"],
    "bv-mb-nv": ["mb", "nv"],
    "v-vice-villa": ["vice-"],
    "v-adv": ["adv"],
    "v-imperfet-ava": ["-ava", "-aves", "-àvem", "-àveu", "-aven"],
    "v-frequents": ["veu", "veure", "vida", "viure", "venir", "verd", "vell", "vent"],
  };
...
function highlightStudyExample(word, ruleId) {
  if (ruleId !== "b-radical-ava") return highlightWordByRule(word, ruleId);
...
function highlightByPattern(text, ruleId) {
  const rules = {
    "b-bl-br": /(bl|br)/gi,
    "b-bu-bur-bus": /^(bu|bur|bus|vu|vul)/i,
    "b-bio-biblio-bi": /^(bio|biblio|bi)/i,
    "b-frequents": /b/gi,
    "bv-mb-nv": /(mb|nv)/gi,
    "v-vice-villa": /^vice/i,
    "v-adv": /adv/gi,
    "v-imperfet-ava": /(ava|aves|àvem|àveu|aven)$/i,
    "v-frequents": /v/gi,
    "homophones": /[bBvV]/,
  };
...
function ensureRuleStats() {
  state.rules.forEach((rule) => {
    if (!state.stats.rules[rule.id]) {
      state.stats.rules[rule.id] = { alpha: 1, beta: 1, hits: 0, misses: 0, attempts: 0 };
    }
...
function loadStats() {
  ...
  try {
    const stored = JSON.parse(localStorage.getItem("ortocat-bv-stats"));
    return {
      ...fallback,
      ...stored,
...
function saveStats() {
  localStorage.setItem("ortocat-bv-stats", JSON.stringify(state.stats));
}
