...
function renderMasked(text, item, isSentence = false) {
  ...
  const aria = `casilla vacía: elige ${displayCase === "upper" ? "B o V mayúscula" : "b o v minúscula"}`;
...
function highlightNorm(text, ruleId, targetWord = "") {
  ...
  const pattern = rules[ruleId] || /[bBvV]/;
...
function loadStats() {
  ...
  try {
    return { ...fallback, ...JSON.parse(localStorage.getItem("ortocat-bv-stats")) };
  } catch {
    return fallback;
  }
}
