...
function bindEvents() {
  ...
  document.querySelector("#clearProgress").addEventListener("click", () => {
    localStorage.removeItem("ortocat-sscz-stats");
  ...
  return `
    <article class="question-card" data-mode="${mode}" data-id="${item.id}">
      ...
      <div class="mini-actions" aria-label="Tria s, ss, c, ç, z o sc">
        ...
        <button class="pick-button pick-c" type="button" data-pick="c">C</button>
        <button class="pick-button pick-c_trencada" type="button" data-pick="ç">Ç</button>
        <button class="pick-button pick-z" type="button" data-pick="z">Z</button>
        <button class="pick-button pick-sc" type="button" data-pick="sc">SC</button>
      </div>
...
function renderMasked(text, item, isSentence = false) {
  ...
  const aria = `casella buida: tria ${displayCase === "upper" ? "S, SS, C, Ç, Z o SC majúscula" : "s, ss, c, ç, z o sc minúscula"}`;
  const blank = `<span class="blank" data-answer="" data-case="${displayCase}" aria-label="${aria}"><span class="blank-placeholder" aria-hidden="true">${displayCase === "upper" ? "SS" : "ss"}</span></span>`;
...
function setAnswer(blank, letter) {
  if (!["s", "ss", "c", "ç", "z", "sc"].includes(letter)) return;
  blank.dataset.answer = letter;
  blank.innerHTML = "";
  blank.textContent = blank.dataset.case === "upper" ? letter.toUpperCase() : letter;
  blank.classList.toggle("is-s", letter === "s");
  blank.classList.toggle("is-ss", letter === "ss");
  blank.classList.toggle("is-c", letter === "c");
  blank.classList.toggle("is-c_trencada", letter === "ç");
  blank.classList.toggle("is-z", letter === "z");
  blank.classList.toggle("is-sc", letter === "sc");
...
function checkSet(mode) {
  ...
  cards.forEach((card) => {
    ...
    if (!selected) {
      feedback.innerHTML = "<strong>Falta respondre.</strong> Tria S, SS, C, Ç, Z o SC abans de corregir aquesta paraula.";
      return;
    }
    answeredCards += 1;
...
function highlightNorm(text, ruleId, targetWord = "") {
  ...
  const rules = {
    "s-inicial-final": /s/gi,
    "ss-entre-vocals": /ss/gi,
    "s-sonora": /s/gi,
    "c-e-i": /(ce|ci)/gi,
    "c-trencada": /ç/gi,
    "z-sonora": /z/gi,
    "sc-cultismes": /sc/gi,
    "homophones": /(ss|sc|s|c|ç|z)/gi,
  };

  const pattern = rules[ruleId] || /(ss|sc|s|c|ç|z)/gi;
...
function loadStats() {
  ...
  try {
    return { ...fallback, ...JSON.parse(localStorage.getItem("ortocat-sscz-stats")) };
  } catch {
    return fallback;
  }
}
...
function saveStats() {
  localStorage.setItem("ortocat-sscz-stats", JSON.stringify(state.stats));
}
